...
const updateStudentProfile = async (formData: FormData) => {
  try {
    ...
    if (resumeImgName && !resumeImgName.success)
      throw new Error("Error in uploading resume");
    if (profileImgName && !profileImgName.success)
      throw new Error("Error in uploading profile");
    ...
    revalidatePath("/"); // Need to revalidate Header too
    return student;
  } catch (error) {
