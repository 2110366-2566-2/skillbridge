...
const fetchInitialData = async (userId:string) => {
    ...
    const tasks: Array<TaskCardType>  = result.map((task) => {return {jobId:task.id, isDeleted:task.isDeleted ,name: task.title, budget: task.budget, description: task.description, category: task.jobTags, applicants: task.acceptNum, maxApplicants: task.maxAcceptNum, startDate: task.startDate, endDate: task.endDate, isPending: task.jobStatus === "NOT_STARTED"}});
