    ...
    <>
      ...
      <div className="flex flex-row justify-between items-center min-h-[70px] pb-4 mb-4 border-b border-slate-300 md:hidden">
        <div className="flex flex-col justify-between w-[210.02px]">
          <div className="font-semibold text-[#313866] h-[3em] line-clamp-2">
            {job?.title}
          </div>
          <div className="flex flex-row justify-between">
            <div className="font-medium text-[11px] text-slate-500">
              {job?.startDate ? FormattedDate(job.startDate) : "ไม่มีกำหนด"} -{" "}
              {job?.endDate ? FormattedDate(job.endDate) : "ไม่มีกำหนด"}
            </div>
            <div className="text-[11px] text-[#838383]">
              รับแล้ว{" "}
              <span className="font-medium">
                {job?.acceptNum} / {job?.maxAcceptNum}
              </span>{" "}
              คน
            </div>
          </div>
        ...
        <div className="flex flex-row justify-end items-center">
          <div className="font-semibold text-[20px] text-[#313866] mr-2">
          ...
          <Image
            ...
            onClick={() => window.open(`/offer/${job.id}`, "_blank", "noopener,noreferrer")}
            role="link"
            tabIndex={0}
          />
        </div>
      ...
      >
        <div className="flex flex-col justify-between h-[340px] w-[268px] p-4">
          <div className="h-[3em] font-semibold text-[29px] text-[#313866] text-wrap line-clamp-2">
            {job?.title}
          </div>
          <div className="font-medium text-[15.5px] text-slate-500 text-wrap line-clamp-1">
            {job?.startDate ? FormattedDate(job.startDate) : "ไม่มีกำหนด"} -{" "}
            {job?.endDate ? FormattedDate(job.endDate) : "ไม่มีกำหนด"}
          </div>
          {job?.jobTags ? (
            <div className="text-[14px] text-slate-800">
              <span className={`inline-block bg-slate-200 rounded py-1 px-2`}>
                {job.jobTags}
              </span>
            </div>
          ) : (
          ...
          <div className="flex flex-col">
            <div className="font-medium text-[10.5px] text-slate-500">
          ...
          <div className="flex flex-row justify-between items-end translate-y-[5px]">
            <div className="text-[13.12px] text-slate-500">
              รับแล้ว{" "}
            ...
            <div className="font-semibold text-[24px] text-[#313866] translate-y-[5px]">
              ฿{job?.budget.toLocaleString()}
            </div>
          </div>
        </div>
