...
export default function LandingHeader(props:Props) {
    ...
    return (
        <div className="flex flex-col justify-center items-center text-slate-50 pt-5 pb-10 gap-3 md:gap-8 md:pb-24 md:pt-16">
            ...
            {isStudent ? (
                <form onSubmit={handleSearch} className='px-2 py-1 md:px-3 md:py-2 text-slate-800 font-bold bg-white rounded-2xl flex justify-center items-center border-b-4 border-pink-400 md:border-b-8 md:text-3xl md:duration-300 md:active:border-b-4 md:active:mt-1'>
                    <input 
                ...
                <Link href="/works" className="text-slate-800 font-bold bg-white rounded-2xl px-24 py-1 border-b-4 border-pink-400 md:border-b-8 md:text-3xl md:py-2 md:px-40 md:hover:border-slate-50 md:hover:text-white md:hover:bg-pink-400 md:duration-300 md:active:border-b-4 md:active:mt-1 ">โพสงานเลย!</Link>
            )}
        </div>
    )
}
