...
export default function JobsHistoryList({ allJobsHistory, userId, isStudent }: { allJobsHistory: AllJobsHistory[]; userId: string; isStudent: boolean; }) {

  return (
    <div className="mt-[20px] w-full md:max-w-[600px] md:mt-0">
      <p className="font-bold text-[24px] text-[#94A3B8] lg:text-[26px]">{isStudent ? "ประวัติการทำงาน" : "ประวัติการจ้างงาน"}</p>
          ...
          <div className="w-full md:max-h-[750px] md:overflow-y-scroll">
            {allJobsHistory.map((job: AllJobsHistory, index: number) => (
              <JobHistory
                ...
                jobId={job.jobId}
                jobTitle={job.title}
                jobPeriod={`${job.estimateStartDate.toLocaleDateString("en-US")} - ${job.estimateEndDate.toLocaleDateString("en-US")}`}
                jobTag={job.jobTag}
                isStudent={isStudent}
              />
            ))}
